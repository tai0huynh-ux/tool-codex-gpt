...
P18-PILOT-001 fixture checkpoint - Add typed desktop orchestration, shared pilot contracts, Live Project Pilot UI, static website verifier, sandboxed preview boundary, and a full fixture counterpart. Published as `960e566`; authenticated live execution and packaged restart acceptance remain open.
...
1. Add packaged smoke coverage and restart-recovery evidence for the pilot.
2. Run packaged UI acceptance from project creation through website verification/preview, recording redacted artifacts only.
3. Attempt authenticated live pilot only if a safe user-opened ChatGPT destination and production Codex runtime are available; otherwise record the exact blocker.

## Latest verification

`pnpm.cmd run verify` passed locally on 2026-07-19 with migration parity, formatting, lint, strict type-check, 191 Vitest tests, two recoverable workflow fixture E2E tests, two Chromium fixture E2E tests, and all 15 buildable workspace projects. `pnpm.cmd run test:project-pilot` passed the complete fixture counterpart. No authenticated live ChatGPT submit, live structured response, writable Codex run, or website generated by production Codex has been claimed in this checkpoint.
