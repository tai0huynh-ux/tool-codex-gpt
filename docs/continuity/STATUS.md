...
P18-PILOT-001 packaged restart checkpoint - Package the Codex native runtime outside ASAR, await adapter disposal during Electron shutdown, restore terminal pilot projections from SQLite without stale run handles, and prove the packaged restart path against isolated app data. Authenticated live execution remains open.
...
1. Confirm a safe user-opened ChatGPT destination with an empty composer is available for authenticated pilot acceptance.
2. If available, run one bounded temporary-repository ChatGPT submit -> structured response -> writable Codex -> website verification/preview flow with explicit approvals and redacted evidence.
3. If the authenticated destination is unavailable, record the exact external condition instead of substituting fixture or packaged restart evidence.

## Latest verification

`pnpm.cmd run verify` passed locally on 2026-07-19 with migration parity, formatting, lint, strict type-check, 195 Vitest tests, two recoverable workflow fixture E2E tests, two Chromium fixture E2E tests, and all 15 buildable workspace projects. `pnpm.cmd run test:internal-beta-uat` passed 46 targeted tests plus two Chromium flows. `pnpm.cmd run test:codex-spike`, `pnpm.cmd run package:win`, `pnpm.cmd run smoke:packaged:win`, and `pnpm.cmd run test:pilot-packaged-restart` passed. Packaged restart evidence is under `artifacts/pilot-restart-acceptance/2026-07-19T07-34-45-337Z/`. No authenticated live ChatGPT submit, live structured response, writable Codex run, or website generated by production Codex has been claimed in this checkpoint.
