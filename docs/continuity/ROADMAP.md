...
## Phase 18 - Live project pilot

- [x] P18-CHATGPT-001 Add explicitly approved ChatGPT submission
...
- [ ] P18-PILOT-001 Deliver the desktop Live Project Pilot vertical slice
  ...
  - Implementation checkpoints: `13ddbd4` (UI, IPC, persistence, verifier, preview boundary), `960e566` (full fixture counterpart), `849812b` (packaged Codex runtime and restart recovery), current checkpoint (validated current-conversation selection); authenticated send/Codex evidence remains open
  - Startup recovery checkpoint: exact inspect/reload/open/retry behavior and ambiguous `dispatching` restoration are implemented and packaged-verified; authenticated send/Codex evidence remains open
  - Chat history archive checkpoint: exact existing-conversation capture, SQLite revision deduplication, 30-second best-effort auto-sync, and lossless project JSON export are implemented and fixture-tested; authenticated send/Codex evidence remains open
  - Canonical destination checkpoint: ChatGPT Project paths are persisted, inspect/status/capture are exact-destination operations, redirects to home fail closed with an explicit unavailable error, and packaged regression gates pass; authenticated send/Codex evidence remains open
  - Multi-connection catalog checkpoint: rendered ChatGPT sidebar conversations and verified Codex project/thread mappings are selectable, projects expand/collapse with five-thread increments, independent pilots can run concurrently, and startup recovery handles multiple exact destinations
  - Safe result bundle checkpoint: a pre-Codex Git baseline distinguishes later commits/working-tree changes, complete Codex reports and accepted files are packaged into an audited ZIP, blocked/deleted files remain manifest-only, and packaged smoke preflights the dependency; automatic browser file upload remains deferred until a separate reviewed boundary is proven
  - Redirect-loop and local-catalog hardening checkpoint: background ChatGPT archive sync never opens a new tab after a redirect, startup recovery opens at most one destination, and the primary pilot control discovers verified Codex projects from bounded local metadata without manual project entry
  - Catalog correctness checkpoint: Codex `thread_name` values persist as display titles, and ChatGPT discovery merges rendered conversations from all open ChatGPT tabs instead of trusting only the active tab
  - Account transfer checkpoint: locally archived rendered history can be bundled and reviewed for a current-account new chat; confirmed inline transfer rebinds the same pilot/Codex target, while oversized ZIP attachment remains explicit and stale extension health fails closed
  - Manual catalog recovery checkpoint: the refresh button may open the allowlisted ChatGPT home once and retry once, background polling remains no-open, canonical title/URL rows are selectable, and Electron prevents duplicate desktop instances; installed live DOM-only selection passed without submission
  - Assisted item-control checkpoint: each reviewed handoff has safe item-scoped deletion; each guided workflow has run/stop/terminal-delete controls; a bounded redacted audit-log dialog shows timestamps and recorded error codes without exposing raw details
  - Operator-context MVP checkpoint: persisted per-pilot ChatGPT/Codex notes support once/repeat routing, and the latest archived conversation can be selected message-by-message with bounded expansion before a reviewed handoff is rebuilt
  - Full control acceptance checkpoint: `PILOT_NOT_DELETABLE` is limited to active Codex runs, ambiguous external effects remain durable and never auto-resend, archive confirmation is modeled in the UI harness, and packaged/installed acceptance passes 14/14 checks across 68 controls; authenticated external send/write evidence remains separately gated
  - Assisted workflow progression checkpoint: `Chạy` advances safely to `context_review_required`, `Chạy lại` creates a linked fresh run for terminal failures/stops, per-workflow controlled notes persist with redacted audit metadata, and packaged/installed acceptance each passes 16/16 checks across 79 controls; authenticated external send/write evidence remains separately gated
