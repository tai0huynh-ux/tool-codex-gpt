...
## 2026-07-22 full control acceptance and archive regression

The remaining UI acceptance failure was a harness defect, not an archive persistence defect: the harness clicked the destructive archive button without accepting its confirmation dialog. The harness now accepts that dialog, and `project-ui.test.ts` proves the selected project is removed while an unrelated project remains. The acceptance sequence also exercises Codex catalog refresh, five-item thread expansion, sample-request/destination toggles, pilot draft deletion, workflow run/stop/terminal delete, log open/refresh/close, persisted-workflow cleanup, and project archive.

Latest evidence: `pnpm.cmd run verify` passed 261 Vitest tests, two workflow fixture tests, two Chromium fixture tests, all workspace builds, and strict formatting/lint/type-check; `pnpm.cmd run test:internal-beta-uat` passed 46 tests plus two Chromium flows; `pnpm.cmd run test:project-pilot` passed; Windows packaging, packaged smoke/native host, fixture restart, and per-user silent installer update passed. Packaged and installed UI acceptance each passed 14/14 checks with 68 controls. Installed no-submit ChatGPT smoke returned `health: ready`, `capturedMessages: 2`, `composerSent: false`, and exact composer cleanup.

## 2026-07-24 assisted workflow recovery

- A guided run must not be inferred as progressing from the click alone. The persisted event sequence is the evidence: `workflow.started_by_user`, `workflow.context.building`, then `workflow.context.review_required`.
- `workflows:run` is deliberately bounded at `context_review_required`; no external send or writable Codex execution occurs in this path.
- Rerun never rewinds a terminal run. `workflows:rerun` creates a new run for the same project, records `sourceWorkflowRunId` in the new start event payload, and copies only local controlled-note records with fresh identities.
- Controlled notes use settings keys `guided-workflow-notes:<workflowRunId>`. Note text is allowed in the reviewed projection but never in audit details or redacted workflow logs. Deleting a terminal run removes its note row in the same transaction.
- Packaged UI acceptance must use exact labels for note controls, wait for each card deletion to settle, and fail the process when any check is `FAIL`; otherwise Playwright substring matching or pending-button state can create false evidence.
