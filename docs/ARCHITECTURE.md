...
The extension service worker reconnects only through a fixed native-host name, receives host-forwarded commands, revalidates expiry/replay/size, and routes DOM work to an exact user-opened ChatGPT tab. A data-free content-ready event wakes the MV3 service worker after browser restart or an old tab load; missing content receivers are injected through the existing `scripting` permission and retried once. It never receives the desktop capability. Electron preload exposes allowlisted status and operation methods; the renderer never receives raw `ipcRenderer`, the capability, or a native port. Windows packaging installs a console launcher, bundled Node-mode host, exact-origin manifest, and per-user Chrome/Edge/Chromium registration in both registry views. The explicitly authorized `nativeMessaging` permission activates this browser-owned port, and the installed no-submit acceptance passed in Edge.
